...
export async function updateContact(id, updates) {
  await fakeNetwork();
  //! lấy ra 1 danh sách contacs
  let contacts = await localforage.getItem("contacts");
  //* tìm object contacs với id tương ứng trong list contact bên trên
  let contact = contacts.find((contact) => contact.id === id);
  //! nếu chưa có thì in ra lỗi
  if (!contact) throw new Error("No contact found for", id);
  //! nếu tìm đc thì gắn contact tìm đc bằng cái object update mà mình thêm vào form khi nãy
  Object.assign(contact, updates);
  //! thực hiện set lại giá trị mảng contacts
  await set(contacts);
  return contact;
}
